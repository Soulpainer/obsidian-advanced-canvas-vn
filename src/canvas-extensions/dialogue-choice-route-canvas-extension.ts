...
export default class DialogueChoiceRouteCanvasExtension extends CanvasExtension {
  ...
  private getChoiceAnchor(
    ...
    // LLM agent change: read the port's position from the DOM LAYOUT tree (offsetTop / offsetHeight
    // walked up to the node element), not from getBoundingClientRect(). Layout offsets are computed
    // from the box model and stay correct even when the node is scrolled off-screen or clipped by
    // Obsidian's lazy rendering — getBoundingClientRect() returned zero / stale boxes in those cases
    // and made choice edges drift. We convert pixel offsets to canvas units via canvas.zoom.
    if (targetEl && nodeEl && canvas.zoom > 0) {
      const centerYPx = this.offsetTopRelativeTo(targetEl, nodeEl) + targetEl.offsetHeight / 2
      const rightXPx = nodeEl.offsetWidth
      return {
        x: bbox.minX + rightXPx / canvas.zoom,
        y: bbox.minY + centerYPx / canvas.zoom,
      }
    }
  ...
  // Used to position a choice port relative to its frame node using layout coordinates.
  private offsetTopRelativeTo(el: HTMLElement, root: HTMLElement): number {
    ...
    while (current && current !== root) {
      ...
      // Stop if we walked past the root without hitting it (defensive).
